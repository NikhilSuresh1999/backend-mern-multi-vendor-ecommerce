  ...
  add_product = async(req,res) =>{
    ...
    form.parse(req, async(err,field,files) =>{
       let {name, category,description, stock,price, discount,shopName,brand} = field;
            // const {images} = files;
            // console.log("________________",images)
            let images = files.images
            if (!images) {
              return responseReturn(res, 400, { error: "No images uploaded" })
            }
    ...
    product_update = async(req,res) =>{
      ...
      try{
        await productModel.findByIdAndUpdate(productId,{
          name, description, stock,price, discount,brand,category,productId,slug
        })
        const product = await productModel.findById(productId)
    ...
    product_image_update = async(req,res) =>{
      ...
      form.parse(req,async (err,field,files)=>{
        ...
        if(err){
          ...
          const publicId = this.getPublicIdFromUrl(oldImage);
